<template>
  <div :class="$style.it">
    <div :class="$style.textA">FREE</div>

    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      :class="$style.icon"
      viewBox="1.22 4 21.57 16"
    >
      <path
        fill="currentColor"
        d="M13.55 5h-3.1c-2.625 0-3.937 0-4.968.497A5 5 0 0 0 3.32 7.511c-.57.992-.663 2.3-.85 4.919l-.246 3.444a2.918 2.918 0 0 0 5.152 2.076l.375-.45c.343-.412.515-.618.71-.784a3 3 0 0 1 1.436-.672c.252-.044.52-.044 1.056-.044h2.095c.536 0 .804 0 1.057.044a3 3 0 0 1 1.436.672c.195.166.367.372.71.784l.375.45a2.917 2.917 0 0 0 5.152-2.075l-.246-3.445c-.187-2.618-.28-3.927-.85-4.92a5 5 0 0 0-2.163-2.013C17.488 5 16.176 5 13.551 5"
        opacity=".12"
      ></path>
      <path
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
        d="M6 11h4M8 9v4m7-1h.01M18 10h.01m-7.561-5h3.102c2.625 0 3.937 0 4.967.497a5 5 0 0 1 2.162 2.014c.57.992.664 2.3.85 4.919l.247 3.444a2.918 2.918 0 0 1-5.152 2.076l-.375-.45c-.343-.412-.515-.618-.71-.784a3 3 0 0 0-1.436-.672C13.851 16 13.584 16 13.047 16h-2.094c-.537 0-.805 0-1.057.044a3 3 0 0 0-1.436.672c-.195.166-.367.372-.71.784l-.375.45a2.918 2.918 0 0 1-5.152-2.075l.246-3.445c.187-2.618.28-3.927.85-4.92a5 5 0 0 1 2.162-2.013C6.511 5 7.824 5 10.45 5"
      ></path>
    </svg>

    <div :class="$style.textB">GAMES</div>
  </div>
</template>

<style lang="css" module>
.it {
  display: flex;
  align-items: center;
  gap: 0.3em;
}

.icon {
  height: 1.1em;
  flex-shrink: 0;
  color: #5adf35;
}

.textA {
  font-family: Nunito;
  font-weight: 800;
  background-image: linear-gradient(to left, #5adf35, #aac 90%);
  text-box-edge: text;
  background-clip: text;
  color: transparent;
}

.textB {
  font-family: Nunito;
  font-weight: 800;
  background-image: linear-gradient(to right, #5adf35, #aac 90%);
  text-box-edge: text;
  background-clip: text;
  color: transparent;
}
</style>
