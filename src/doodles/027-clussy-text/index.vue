<script setup lang="ts">
import { Bento } from '../../components';
</script>

<template>
  <!--
    contain:layout/paint prevents mix-blend-mode
    to work correctly.
  -->
  <Bento.Cell w="12" h="3" style="contain: size">
    <div :class="$style.textureA" />
    <div :class="$style.textureB" />

    <div :class="$style.it">
      <div :class="$style.backdrop"></div>
      <div :class="$style.text">clown pussy fun and juicy</div>
    </div>
  </Bento.Cell>
</template>

<style module>
.it {
  font-family: 'Shantell Sans';
  font-size: 2rem;
  display: grid;

  > * {
    grid-area: 1/1;
  }
}

.backdrop {
  width: 100%;
  height: 100%;
  background-image: linear-gradient(90deg, #24f4ff, #ff47ff, #ff0);
  background-image: linear-gradient(90deg, #5ae4ff, #ff64ff, #ffeb00);
  border-radius: 99px;
  z-index: 1;
  filter: blur(0.1em);

  &::after,
  &::before {
    content: '';
    background-image: inherit;
    border-radius: inherit;
    width: 100%;
    height: 100%;
    display: block;
  }

  &::before {
    filter: blur(0.8em);
  }

  &::after {
    position: relative;
    top: -100%;
    filter: blur(0.6em);
  }
}

.text {
  z-index: 1;
  color: white;
  line-height: 0.75em;
  text-box: trim-both ex alphabetic;
  filter: blur(0.0125em);
  padding: 0.3em 0.3em;
  text-shadow:
    0 0 0.4em white,
    0 0 0.1em white,
    0 0 0.05em white;
}

.textureA {
  position: absolute;
  inset: 0;
  z-index: 100;
  pointer-events: none;
  mix-blend-mode: soft-light;
  background-position: center;
  background-attachment: fixed;

  opacity: 0.2;
  background-image: url('data:image/png;base64,');
}

.textureB {
  position: absolute;
  width: 100%;
  height: 100%;
  z-index: 100;
  pointer-events: none;
  mix-blend-mode: soft-light;
  background-position: center;
  background-attachment: fixed;

  background-image: url("data:image/svg+xml,\
    <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 700 700' width='600' height='600'>\
      <defs>\
        <filter id='noise' x='-20%' y='-20%' width='140%' height='140%' filterUnits='objectBoundingBox' primitiveUnits='userSpaceOnUse' color-interpolation-filters='linearRGB'>\
          <feTurbulence type='fractalNoise' baseFrequency='0.164' numOctaves='4' seed='15' stitchTiles='stitch' width='100%' height='100%' result='turbulence'/>\
          <feSpecularLighting surfaceScale='15' specularConstant='0.75' specularExponent='20' lighting-color='%23FFFFFF' width='100%' height='100%' in='turbulence' result='specularLighting'>\
            <feDistantLight azimuth='3' elevation='100'/>\
          </feSpecularLighting>\
        </filter>\
      </defs>\
      <rect width='700' height='700' fill='transparent'/>\
      <rect width='700' height='700' fill='%23CF3266' filter='url(%23noise)'/>\
      </svg>\
  ");
}
</style>
